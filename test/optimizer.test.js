...
test('3.1 cost model: costo crece con ops (tokens/latency/calls); utility = quality / cost', () => {
  ...
  // el espacio incluye planes multi-op (B/D/E/F) más costosos que el mínimo
...
test('3.2 selección por utilidad (max quality/cost) en definitions y filename', () => {
  for (const qt of ['definitions', 'filename']) {
    const r = optimize({ query_type: qt, target: { kind: 'symbol', name: 'x' } });
    const best = r.plans.reduce((m, p) => (p.utility > m.utility ? p : m), r.plans[0]);
    assert.equal(r.selected, best.id, `selected = plan de max-utility (${qt})`);
  }
});
